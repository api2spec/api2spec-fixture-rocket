...
fn update_post(id: i32, post: Json<Post>) -> Result<Json<Post>, rocket::http::Status> {
    if id > 100 {
        return Err(rocket::http::Status::NotFound);
    }
    let mut updated = post.into_inner();
    updated.id = id;
    Ok(Json(updated))
}

#[delete("/posts/<id>")]
fn delete_post(id: i32) -> rocket::http::Status {
    if id > 100 {
...
/// Creates the Rocket instance with all routes mounted.
